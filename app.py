...
if parsed_df is not None and not parsed_df.empty:
    ...
    with tab1:
        ...
        if view_mode == "전체 시간표 (가로: 학급 / 세로: 요일·교시)":
            ...
            st.markdown(build_weekly_html_table(parsed_df[parsed_df["학급"] == target_cls], target_cls), unsafe_allow_html=True)
        else:
            target_t = st.selectbox("👨‍🏫 교사 선택", teacher_list)
            st.markdown(build_weekly_html_table(parsed_df[parsed_df["교사"] == target_t], f"{target_t} 선생님"), unsafe_allow_html=True)
    ...
    with tab2:
        ...
        if r1 is not None and r2 is not None:
            if st.button("📩 수업 교체 요청 등록", use_container_width=True):
                log_entry = {
                    "cls1": selected_cls, "date1": str(date_a), "period1": int(r1["교시"]), "subj1": r1["과목"], "teacher1": r1["교사"],
                    "cls2": selected_cls, "date2": str(date_b), "period2": int(r2["교시"]), "subj2": r2["과목"], "teacher2": r2["교사"]
                }
