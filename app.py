...
if "copied_cell" not in st.session_state:
...
# 팝업 다이얼로그 (셀 편집 팝업 모달)
@st.dialog("📌 수업 원클릭 편집 / 교체 / 대강")
def open_cell_modal(target_cell, parsed_df):
    st.markdown(f"#### [{target_cell['cls']}] {target_cell['day']}요일 {target_cell['period']}교시")
    st.write(f"현재 수업: **{target_cell['subj']} ({target_cell['teacher']})**")
    
    if st.session_state.copied_cell:
        cp = st.session_state.copied_cell
        st.info(f"📋 **현재 복사된 수업**: {cp['subj']} ({cp['teacher']})")

    st.markdown("---")
    
    col1, col2 = st.columns(2)
    with col1:
        if st.button("📋 이 수업 복사", use_container_width=True):
            st.session_state.copied_cell = target_cell
            st.success(f"[{target_cell['subj']}] 수업이 복사되었습니다!")
            st.rerun()

        if st.button("🗑️ 이 수업 삭제 (휴강)", use_container_width=True):
            log_entry = {
            ...
            st.session_state.sub_logs = load_sub_logs()
            st.success("해당 수업이 휴강 처리되었습니다.")
            st.rerun()

    with col2:
        if st.session_state.copied_cell:
            cp = st.session_state.copied_cell
            if st.button("📥 복사본 덮어쓰기", use_container_width=True):
                log_entry = {
                    "날짜": target_cell["date"], "요일": target_cell["day"], "교시": target_cell["period"],
            ...
            if st.button("🔀 복사본과 위치 교체", use_container_width=True):
                swap_entry = {
...
if parsed_df is not None and not parsed_df.empty:
    ...
    if is_admin:
        tab1, tab2, tab3, tab4 = st.tabs(["🗓️ 전체/학급/교사 시간표", "🔄 수업 위치 맞교환 신청", "📝 대강 지정 및 사유 (관리자 전용)", "📊 교사 시수 & 대강 수당 (관리자 전용)"])
    else:
    ...
    with tab1:
        ...
        if is_admin and view_mode == "전체 시간표 (가로: 학급 / 세로: 요일·교시)":
            st.markdown("##### 💡 **관리자 안내**: 수정/교체/복사할 셀을 아래 선택창에서 고르면 스마트 팝업창이 바로 열립니다.")
                ...
                for p in range(1, 8):
                    for c in classes:
                        ...
                        if not c_df.empty:
                            ...
                            sub_key = (d_str, c, p)
                            if sub_key in sub_dict:
                                display_txt = f"[{c}] {d}요일 {p}교시: {subj} (원래: {teacher} ➔ 대강: {sub_dict[sub_key]['대강교사']})"
                            else:
            ...
            with col_btn:
                st.write("")
                if st.button("⚙️ 셀 편집 팝업 열기", use_container_width=True):
                    open_cell_modal(cell_map[selected_label], parsed_df)
