...
# URL 신호 감지하여 백엔드 DB 실시간 업데이트
qp = st.query_params
if "act" in qp:
    action_type = qp.get("act")
    t_date = qp.get("date")
    t_cls = qp.get("cls")
    t_period = int(qp.get("period", 1))
    t_subj = qp.get("subj", "")
    t_teacher = qp.get("teacher", "")

    c_date = qp.get("c_date", "")
    c_cls = qp.get("c_cls", "")
    c_period = int(qp.get("c_period", 1)) if qp.get("c_period") else 1
    c_subj = qp.get("c_subj", "")
    c_teacher = qp.get("c_teacher", "")
    ...
    if action_type == "DELETE":
        ...
        log_entry = {
            "날짜": t_date, "요일": day_kr, "교시": t_period,
            "학급": t_cls, "원교사": t_teacher, "대강교사": "휴강", "대강사유": "관리자 삭제",
            "단가": 0, "주차": st.session_state.week_offset
        }
        save_sub_log(log_entry)
        st.session_state.sub_logs = load_sub_logs()
        st.toast(f"🗑️ [{t_cls}] {t_period}교시 수업 삭제 완료")
        st.rerun()

    elif action_type == "PASTE_OVERWRITE":
        day_kr = ["월","화","수","목","금"][pd.to_datetime(t_date).weekday()]
        log_entry = {
            "날짜": t_date, "요일": day_kr, "교시": t_period,
            "학급": t_cls, "원교사": t_teacher, "대강교사": c_teacher,
            "대강사유": f"복사 배치({c_subj})", "단가": st.session_state.hourly_rate, "주차": st.session_state.week_offset
        }
        save_sub_log(log_entry)
        st.session_state.sub_logs = load_sub_logs()
        ...
        st.session_state.swap_logs = load_swap_logs("APPROVED")
        st.toast(f"🔀 [{c_cls}] {c_period}교시 ↔ [{t_cls}] {t_period}교시 맞교환 완료")
        st.rerun()
